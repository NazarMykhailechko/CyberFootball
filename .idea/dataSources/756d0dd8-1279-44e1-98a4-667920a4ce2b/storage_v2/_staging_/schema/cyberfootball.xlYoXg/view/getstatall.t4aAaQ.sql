...
from (select `q`.`Season`                    AS `Season`,
      ...
      from (select `getteams`.`season`   AS `Season`,
            ...
            from (`cyberfootball`.`datacyber`
                     join `cyberfootball`.`getteamsall` on ((`cyberfootball`.`datacyber`.`season` = `getteamsall`.`season`)))
            where (((`cyberfootball`.`datacyber`.`nickhomelong` like concat(`getteamsall`.`Nick9`, '%')) and
                   ...
                   ((str_to_date(left(`cyberfootball`.`datacyber`.`dates`, 8),
                    ...
                    (`cyberfootball`.`datacyber`.`nickawaylong` like concat(`getteamsall`.`Nick9`, '%'))))
            group by `getteamsall`.`season`, `getteamsall`.`Nick`) `q`) `qq`
order by `qq`.`Nick`;
