...
select
       ...
       `qq`.`TOTAL`               AS `TOTAL`,
       `qq`.`W`                   AS `W`,
       concat(format(W /TOTAL*100,1),'%')  AS `Wp`,
       `qq`.`D`                   AS `D`,
       concat(format(D /TOTAL*100,1),'%')  AS `Dp`,
       `qq`.`L`                   AS `L`,
       concat(format(L /TOTAL*100,1),'%')  AS `Lp`,
       `qq`.`TB`                  AS `TB`,
       concat(format(TB /TOTAL*100,1),'%')  AS `TBp`,
       `qq`.`TM`                  AS `TM`,
       concat(format(TM /TOTAL*100,1),'%')  AS `TMp`
